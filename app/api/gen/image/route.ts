...
export async function POST(req: NextRequest) {
    try {
        ...
        if (typeof optionsRaw === "string" && optionsRaw.trim().length) {
            try { userOptions = JSON.parse(optionsRaw); } catch { /* ignore */ }
        }
        ...
        if (debit.count <= 0) {
            return NextResponse.json({ error: "اعتبار کیف پول کافی نیست." }, { status: 402 }); // Payment Required
        }

        // 2) Create prediction; refund on failure
        let prediction:
            { id: string; status: string; output: unknown; urls: { get: string; cancel: string; web: string } };
        try {
        ...
        try {
            const job = await prisma.imageJob.create({
                data: {
                    ...
                    replicateCancelUrl: prediction.urls?.cancel ?? `https://api.replicate.com/v1/predictions/${prediction.id}/cancel`,
        ...
        const msg = err?.code === 401 ? "دسترسی غیرمجاز." : "در ایجاد درخواست ساخت تصویر خطایی رخ داد.";
...
export async function GET(req: NextRequest) {
    try {
        ...
        const job = await prisma.imageJob.findFirst({
            ...
            select: {
                id: true, status: true, progress: true, createdAt: true, imageUrl: true,
        ...
        const makeStreamUrl = () =>
            job.imageUrl
                ? new URL(`/api/gen/image/stream?jobId=${encodeURIComponent(String(job.id))}`, req.nextUrl.origin).toString()
                : undefined;
        ...
        const updated = await prisma.$transaction(async (tx) => {
            ...
            return tx.imageJob.update({
                ...
                select: { status: true, progress: true, createdAt: true, imageUrl: true, id: true },
            });
        });
        ...
        const etaSeconds =
            updated.status === "SUCCEEDED" || updated.status === "FAILED" ? 0 : Math.max(1, 20 - elapsedSec);
        ...
        return NextResponse.json({
            ...
            imageUrl: updated.imageUrl
                ? new URL(`/api/gen/image/stream?jobId=${encodeURIComponent(String(updated.id))}`, req.nextUrl.origin).toString()
                : undefined,
            ...(updated.status === "FAILED" ? { error: "ساخت تصویر ناموفق بود." } : {}),
        });
    } catch {
        // unchanged fallback
        const jobIdRaw = req.nextUrl.searchParams.get("jobId");
        ...
        return NextResponse.json(
            {
                ...
                imageUrl: safe?.imageUrl
                    ? new URL(`/api/gen/image/stream?jobId=${encodeURIComponent(String(safe.id))}`, req.nextUrl.origin).toString()
                    : undefined,
