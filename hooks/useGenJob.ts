...
type UseGenJobOptions<ResultKey extends string> = {
    endpoint: string; // e.g. "/api/gen/image"
    resultKey: ResultKey; // "imageUrl"
        ...
        try {
            ...
            pollTimerRef.current = setInterval(async () => {
                try {
                    ...
                    setEta(typeof data.etaSeconds === "number" ? data.etaSeconds : undefined);
                    ...
                    if (data.status === "SUCCEEDED") {
                        ...
                        clearPolling();
                        const errMsg =
                            (typeof data.error === "string" && data.error) || failErrorFallback;
