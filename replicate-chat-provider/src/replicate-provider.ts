...
function parseTokenUsageFromLogs(logs?: string): {
    ...
    if (!logs) return {};
    const patterns: Array<[RegExp, 'inputTokens' | 'outputTokens' | 'totalTokens']> = [
...
// NEW: merge usage from logs and metrics
function mergeUsage(a: { inputTokens?: number; outputTokens?: number; totalTokens?: number },
...
async function fetchAsBytes(url: string, abortSignal?: AbortSignal): Promise<{ bytes: Uint8Array; contentType: string | null; contentLength: number | null; }> {
        ...
        if (head.ok) {
            ...
            if (len) {
                ...
                if (Number.isFinite(n) && n > 262_144 /* 256 KiB */) {
                    return { bytes: new Uint8Array(0), contentType: type, contentLength: n };
                }
            }
        }
    } catch {
    ...
    return { bytes: buf, contentType: type, contentLength: Number.isFinite(len) ? len : buf.byteLength };
}
...
async function filePartToDataOrHttpUrl(
    ...
    if (typeof part.data === 'string') {
        ...
        if (/^https?:\/\//i.test(str)) {
            const { bytes, contentType, contentLength } = await fetchAsBytes(str, abortSignal);
...
async function extractUserMediaDataUrls(
    ...
    for (const msg of prompt ?? []) {
        ...
        for (const p of parts) {
            if (p?.type === 'file') {
                const urlOrData = await filePartToDataOrHttpUrl(p as FilePart, abortSignal);
                out.push(urlOrData);
            }
        }
    }
...
function toChatMessagesArray(prompt: PromptT): Array<{ role: string; content: string }> {
...
class ReplicateLanguageModel implements LanguageModelV2 {
    ...
    constructor(
        ...
        this.baseURL = withoutTrailingSlash(config.baseURL) ?? 'https://api.replicate.com';
    ...
    private async buildCreateBody(options: LanguageModelV2CallOptions) {
        ...
        const fileDataUrls = await extractUserMediaDataUrls(options.prompt, options.abortSignal);
    ...
    private async createPrediction(options: LanguageModelV2CallOptions) {
        ...
        const { value: prediction } = await postJsonToApi<z.infer<typeof predictionSchema>>({
            url,
            headers: {
                Authorization: `Bearer ${this.apiKey}`,
                'Content-Type': 'application/json',
            ...
            failedResponseHandler: createJsonErrorResponseHandler<z.infer<typeof errorSchema>>({
                errorSchema,
        ...
        if (!prediction.urls?.stream) {
            throw new Error('Replicate response did not include urls.stream. Ensure the model supports streaming.');
        }
    ...
    async doStream(options: LanguageModelV2CallOptions): Promise<DoStreamResult> {
        ...
        const transformer = new TransformStream<Uint8Array, LanguageModelV2StreamPart>({
            start(controller) {
                controller.enqueue({ type: 'stream-start', warnings: [] as LanguageModelV2CallWarning[] });
                controller.enqueue({
            ...
            transform: (() => {
                ...
                let controllerRef: TransformStreamDefaultController<LanguageModelV2StreamPart> | null = null;
                ...
                const parser = createParser({
                    onEvent: (evt) => {
                        ...
                        if (event === 'error') {
                            try {
                                const parsed = data ? JSON.parse(data) : { detail: 'Unknown error' };
                                controllerRef.enqueue({ type: 'error', error: parsed });
                            } catch {
                                controllerRef.enqueue({ type: 'error', error: data ?? 'Unknown error' });
                            }
                            return;
                        }

                        if (event === 'done') {
                            if (textStarted) controllerRef.enqueue({ type: 'text-end', id: textBlockId });
                ...
                return (chunk: Uint8Array, controller: TransformStreamDefaultController<LanguageModelV2StreamPart>) => {
                ...
                case 'error': {
                    const message =
                        typeof part.error === 'string'
                            ? part.error
                            : ((part.error as { detail?: string })?.detail as string) ?? 'Unknown error';
...
export type ReplicateProvider = ProviderV2 & {
    (modelId: ReplicateModelId, settings?: ReplicateModelSettings): LanguageModelV2;
    languageModel: (modelId: ReplicateModelId, settings?: ReplicateModelSettings) => LanguageModelV2;
};

export function createReplicate(providerSettings: ReplicateProviderSettings = {}): ReplicateProvider {
    ...
    const baseURL = withoutTrailingSlash(providerSettings.baseURL ?? 'https://api.replicate.com')!;

    const factory = (modelId: ReplicateModelId, settings?: ReplicateModelSettings): LanguageModelV2 =>
        new ReplicateLanguageModel(modelId, { apiKey, baseURL, headers: providerSettings.headers }, settings);

    const provider: ReplicateProvider = Object.assign(factory as ReplicateProvider, {
        languageModel: factory,
        textEmbeddingModel: () => {
            throw new Error('Replicate provider: textEmbeddingModel is not implemented.');
        },
        imageModel: () => {
