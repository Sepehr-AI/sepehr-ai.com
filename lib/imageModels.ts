...
export type ImageModelDto = Pick<
    ImageModelDb,
    "id" | "code" | "name" | "description" | "cost" | "imageInput" | "defaultOptions" | "showCaseImage"
...
export const getImageModelsMap = async () => {
    ...
    try { revalidateTag("imageModelsForWeb"); } catch { }
    return map;
};
...
export type ImageModelPricingDto = Pick<
    ImageModelDto,
    "code" | "name" | "description" | "companyWebsite" | "imageInput" | "ratios" | "defaultOptions" | "showCaseImage"
> & { creditCostPerImage: number };
...
export const getImageModelsForWeb = unstable_cache(
    async () => {
        ...
        const modelsForWeb = (await getImageModels()).map(
            ({ code, name, description, cost, companyWebsite, imageInput, ratios, defaultOptions, showCaseImage, }) => ({
                code,
                name,
                ratios,
                imageInput,
                ...
                companyWebsite,
                showCaseImage,
                creditCostPerImage: usdToCredit(cost, true),
            }),
        );
